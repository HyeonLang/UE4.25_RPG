...
| **주요 기능** | 멀티플레이 동기화, 캐릭터 교체 시스템, 액션 시스템(일반공격/스킬/궁극기), 상호 작용, 미니맵 및 UI 개발, DB 서버 기반 로그인 시스템 |
...
| **캐릭터 교체 시스템** | 플레이어가 3명의 캐릭터를 소환, 동시 소환 및 교체 가능 | `Delegate` 기반 콜백<br>컨트롤러의 `Possess` 및 `UnPossess` | 
| **액션 시스템** | Unreal GAS를 모티브로 한 일반 공격, 스킬, 궁극기 등의 모듈화된 액션 클래스 | 기반 클래스 상속<br>데이터 에셋 사용<br>우선순위 큐 - 자동 타겟팅 시스템<br>애니메이션 몽타주 및 노티파이  |
...
| **DB 서버 기반 로그인 시스템** | Flask 서버와 MySQL 연동을 통한 로그인 인증 | `HTTP Post`, `MySQL`, `Flask` |
...
### 🎯 목표
- 2인 이상의 유저가 멀티플레이를 할 수 있는 환경 제작.
- 유저간 데이터가 **일관성**있고 **실시간으로 동기화** 되어야함.
- 코드가 **꼬이지 않고** 유지보수가 용이해야함.
...
### 🎯 목표
- 플레이어는 다수의 캐릭터를 다룰 수 있어야 하므로 컨트롤러 / 캐릭터의 역할을 알맞게 분리.
- 캐릭터의 기능에 따라 컴포넌트를 분리하여 코드의 응집도 상승 및 결합도를 낮춤.
- 유저의 입력 키가 동일해도 다수의 캐릭터가 각자의 기능을 수행하도록 함.
...
### 🎯 목표
- 캐릭터의 교체에 따라 카메라, UI 등 유저 레벨에서 보여지는 것들을 적절히 유지하거나 변경.
- 캐릭터 교체시 캐릭터의 상태 변화가 필수적. 상태 변화 갱신 및 원상복구가 완벽하게 되어야함.
- 멀티플레이에서 다른유저와의 동기화가 잘 이루어져야함.
...
- `Delegate.Broadcast()`로 협동 공격 교체 후 UnpossessCharacter() 실행
...
### 🎯 목표
- 캐릭터가 다수인 만큼 액션을 쉽게 제작할 수 있어야하며 동작이 수행되어야 함.
- 액션의 실행-종료에 어긋남이 없고 액션간 겹침에 대한 오류가 없어야함.
- 자신의 실행을 다른 유저도 볼 수 있어야하며 액션 효과의 동기화가 잘 이루어져야 함.
